...
| Plus/支付 | `plus/`, `MonicaPlusScreen`, `PaymentScreen` | StoreKit 2 + 现有 Plus 映射 | 开发中 | App 会话已新增 StoreKit 2 product entitlement 与旧 Plus/CDK entitlement 的统一 Monica Plus 状态映射第一版；Settings 已显示 Plus 激活状态和 Android 同口径功能权益列表，状态文案不泄漏 transaction id、original transaction id、license id 或 receipt/CDK 原始标识；Apple IAP 购买/恢复、App Store receipt 校验、CDK 网络校验、权益持久化和签名真机支付验收仍待后续 |

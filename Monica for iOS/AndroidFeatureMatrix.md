...
| KDBX/KeePass | `LocalKeePass*`, `keepass/` | KDBX 读写兼容 | 开发中 | Storage 层已按 Android `KeePassFormatInspector` 口径识别 KDBX、旧版 KDB 和未知格式；旧 KDB 会提示需先在 KeePassDX/KeePassXC 另存为 `.kdbx`，KDBX 预览会进入等待密码/密钥文件解锁状态；设置页已接入 KDBX 文件检查入口，预览不会写入当前 MDBX vault；完整打开、编辑、保存、回收站、附件和云文件源仍待后续 |

...
KDBX/KeePass 进展备注：Storage 已新增 KDBX payload cipher 边界并完成 AES-256-CBC 解密第一版。`DefaultKeePassKdbxPayloadCipher` 会用 KDBX master key 与 encryption IV 解密 AES payload，支持合法 PKCS#7 padding 剥离；ChaCha20 payload 已在后续节点补齐，Twofish/未知 cipher 仍返回明确脱敏 unsupported。默认 payload decryptor 会按 KDF -> master key -> payload cipher 顺序推进；这仍不声明 Argon2 或 KDBX 保存已完成。

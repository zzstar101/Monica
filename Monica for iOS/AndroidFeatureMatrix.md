...
| 密码历史/时间线 | `TimelineScreen`, `PasswordHistoryDao`, `OperationLogDao` | 历史版本和操作时间线 | 开发中 | App 会话内已记录 `login/note/totp/card/identity` 条目创建、更新、删除、恢复操作时间线，并在设置页显示最近事件；事件只包含动作、类型、条目 ID、标题和时间，不包含密码、用户名、笔记正文、TOTP seed、卡号/CVV、证件号等秘密；持久化历史版本、跨会话审计和版本恢复待后续 |

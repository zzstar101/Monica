...
KDBX/KeePass 进展备注：Storage 已新增 KDBX file assembler 第一版，可复用既有 KDBX header bytes 与新写出的 payload section 组装完整 `.kdbx` bytes，并通过现有 envelope parser 和默认 reader 重新打开验证。该节点仍不声明 KDBX header 生成/改写、原文件原位保存、云文件源 writeback 或 KeePass 原生回收站还原语义已完成。

KDBX/KeePass 进展备注：Storage 已新增 KDBX4 header writer 第一版，可生成 KDBX4 外层 header bytes，覆盖 AES-256、无压缩/GZip、AES-KDF、Argon2d、Argon2id、master seed、encryption IV 和 inner random stream key/id；生成结果可由现有 header parser 解析，并接上 Argon2id KDF、AES payload writer、KDBX4 payload section writer 与 file assembler 后由默认 reader 重新打开。错误文案和摘要不泄漏 salt、seed、IV、密码、derived/master key 或 XML 明文。该节点仍不声明既有 header 原位改写、ChaCha20/Twofish payload 加密、原 `.kdbx` 文件系统/云端 writeback 或 KeePass 原生回收站还原语义已完成。

KDBX/KeePass 进展备注：Storage 已新增 KDBX4 writeback coordinator 第一版，把 KeePass snapshot -> XML payload -> GZip -> Argon2id KDF -> AES-256-CBC payload 加密 -> KDBX4 header/payload section -> 完整 `.kdbx` bytes 串成可调用保存 pipeline，并由默认 reader 重新打开验证。为避免写出当前 inner protected stream 无法解码的 XML，coordinator 第一版会把 XML protected values 写成普通 Value，依赖外层 KDBX 加密保护；真正的 protected value 加密写回、ChaCha20/Twofish payload 加密、既有 header 原位改写、原 `.kdbx` 文件系统/云端 writeback 和 KeePass 原生回收站还原语义仍待后续。
...
KDBX/KeePass 进展备注：App 层已新增 KeePass decoded 附件内容替换第一版。`replaceKeePassReadOnlyAttachmentContent` 会在当前只读 snapshot 中定位 entry/attachment，替换 decoded bytes、originalSize 和 `sha256:` 内容 hash，清空旧导入计划，并让后续 KDBX4 writeback request 带着更新后的附件内容进入 Storage 保存 pipeline；失败路径返回脱敏的“未找到条目/附件”提示并保留原 snapshot。Settings 已把当前只读预览中带 decoded attachment 的 KeePass 条目接入“替换并写回”文件选择入口，用户选中文件后会读取本地文件内容、清洗文件名、更新 snapshot 附件，并立即写回本地源 `.kdbx`。该节点仍不声明 protected binary value 加密写回、云文件源 writeback 或 KeePass 原生回收站还原语义已完成。

KDBX/KeePass 进展备注：App/Settings 层已新增 KeePass 附件编辑候选、搜索、新增和删除写回第一版。`keePassAttachmentEditCandidates(matching:)` 会从当前 snapshot 返回全量附件候选，不再受只读预览前三条/前两个附件限制；候选只包含 entry id、attachment id、标题、用户名、分组路径、清洗文件名、媒体类型、大小和删除态，不包含 decoded attachment bytes、content hash、密码、notes 或 XML/KDBX bytes。Settings 现在可按附件/条目/分组/媒体类型搜索全量候选，并对本地源 `.kdbx` 执行“替换并写回”“添加附件并写回”“删除并写回”；App 层会重建 snapshot 附件数组、更新 attachmentCount，并复用 KDBX4 writeback request 与本地文件写回服务。该节点仍不声明 protected binary value 加密写回、云文件源 writeback、既有 header 原位改写或 KeePass 原生回收站还原语义已完成。

...
int monica_twofish_encrypt_cbc(
    const uint8_t *key,
    size_t keyLength,
    const uint8_t *iv,
    size_t ivLength,
    ...
    keyInstance keyInstance;
    cipherInstance cipherInstance;
    int keyStatus = makeKey(&keyInstance, DIR_ENCRYPT, 256, keyHex);
    if (keyStatus != TRUE) {
        return keyStatus;
    }
    int cipherStatus = cipherInit(&cipherInstance, MODE_CBC, ivHex);
    ...
    int encryptedBits = blockEncrypt(
        &cipherInstance,
        &keyInstance,
        (uint8_t *)input,
        (int)(inputLength * 8),
    ...
    return encryptedBits == (int)(inputLength * 8) ? TRUE : encryptedBits;
}
